export const FA_UNICODE: Record<string, string> = {
  "0": "30",
  "1": "31",
  "2": "32",
  "3": "33",
...
export function faGlyph(faClass: string): string | null {
  const match = faClass.match(/fa-([\w-]+)\s*$/);
  if (!match) return null;
  const unicode = FA_UNICODE[match[1]];
  return unicode ? String.fromCodePoint(parseInt(unicode, 16)) : null;
}