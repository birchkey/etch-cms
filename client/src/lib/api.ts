...
export const assetsApi = {
  ...
  // Absolute form of `url` — what you paste outside the admin UI. Built on the configured
  // files domain when there is one, matching what the public API emits, so a copied link
  // does not silently point at the admin hostname. Note this resolves for anyone only when
  // the asset is marked public; otherwise it works in a signed-in admin's browser (session
  // cookie) and 401s for everyone else.
  absoluteUrl: (r2Key: string, assetsHostname?: string) =>
    `${assetsHostname ? `https://${assetsHostname}` : window.location.origin}${assetsApi.url(r2Key)}`,
};
